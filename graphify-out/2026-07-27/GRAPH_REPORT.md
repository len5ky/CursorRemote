...
## Corpus Check
- 88 files · ~152,684 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1372 nodes · 2553 edges · 99 communities (92 shown, 7 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 35 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f8552122`
...
2. `CdpClient` - 42 edges
3. `CommandExecutor` - 42 edges
4. `CDPBridge` - 36 edges
5. `StateManager` - 34 edges
6. `WindowMonitor` - 33 edges
7. `ServerManager` - 29 edges
8. `CursorState` - 29 edges
9. `cleanTabTitle()` - 23 edges
...
Cohesion: 0.06
Nodes (36): CDPTarget, discoverTargets(), main(), CDPTarget, main(), CDPTarget, main(), CDPTarget (+28 more)

### Community 1 - "server/types.ts"
Cohesion: 0.05
Nodes (66): contentHash(), elapsed(), main(), RecordLine, RecordLineV1, RecordLineV2, stripHtml(), activityFromThoughtTail() (+58 more)
...
Cohesion: 0.10
Nodes (31): ALLOWED_EXACT, ALLOWED_PREFIXES, ALLOWED_ZIP_ROOT, DEV_ROOT, FORBIDDEN_PATTERNS, main(), PKG_PATH, REQUIRED_FILES (+23 more)
...
Cohesion: 0.08
Nodes (19): ActionClickTargetResult, actionSearchRoot(), buttonLikeCandidates(), elementLabelMatches(), firstSelectorSegment(), isElementRoot(), matchingResolvedActionTarget(), MODEL_ITEM_COLLECTOR_JS (+11 more)

### Community 8 - "telegram-raw/index.ts"
Cohesion: 0.12
Nodes (14): getThreadIdFromContext(), handleCallbackQuery(), handleRegister(), handleTextMessage(), RegisterDeps, grammyApiAdapter(), grammyCtxToBotCtx(), TelegramTransport (+6 more)

### Community 9 - "commands.ts"
Cohesion: 0.20
Nodes (24): ACTION_SELECTORS, CommandDeps, doCleanupInBackground(), doDedupeInBackground(), doPurgeInBackground(), doSyncInBackground(), ensureTopicWindow(), genId() (+16 more)

### Community 10 - "cleanTabTitle"
Cohesion: 0.18
Nodes (8): cleanTabTitle(), handleDedupe(), makeRuntimeKey(), makeTitleKey(), normalizeWindowTitle(), TopicManager, TopicMapping, CursorWindow

### Community 11 - "CursorRemote"
Cohesion: 0.08
Nodes (25): 1. Install the Extension, 2. Enter Your License Key, 3. Launch Cursor with CDP Enabled, 4. Server Auto-Starts, 5. Configure Networking and Connect, Accessing from another device, Bot Commands, CursorRemote (+17 more)

### Community 12 - "4. State Model"
Cohesion: 0.09
Nodes (23): 4.10 Questionnaire, 4.11 QuestionnaireQuestion, 4.12 QuestionnaireOption, 4.1 CursorState (top-level), 4.2 AgentStatus, 4.3 ChatElement (discriminated union), 4.4 ChatTab, 4.5 ModeInfo (+15 more)

### Community 13 - "Telegram Transport — Architecture Document"
Cohesion: 0.09
Nodes (22): 1. Component Overview, 2. Module Structure, 3.1 Outbound: Cursor → Telegram, 3.2 Inbound: Telegram → Cursor, 3. Data Flow, 4.1 TelegramTransport (`index.ts`), 4.2 Formatter (`formatter.ts`), 4.3 TopicManager (`topic-manager.ts`) (+14 more)
...
Cohesion: 0.14
Nodes (9): __dirname, __filename, RateLimitEntry, Relay, CommandPayload, ServerConfig, createWebappSessionStore(), parseSessionCookie() (+1 more)

### Community 16 - "base.ts"
Cohesion: 0.14
Nodes (17): AuthState, BOT_COMMANDS, RegisteredUser, handleResync(), escapeHtml(), formatApprovals(), formatComposerQueue(), formatPlan() (+9 more)
...
Cohesion: 0.21
Nodes (3): CDPBridge, parseCdpTitle(), main()
...
### Community 29 - "TelegramApiClient"
Cohesion: 0.14
Nodes (4): TgCallbackQuery, TgMessage, TelegramApiClient, TgInlineButton
...
Cohesion: 0.17
Nodes (12): 2. User Stories, TG-10: Typing Indicator, TG-1: Topic-Based Monitoring, TG-2: Live Chat Streaming, TG-3: Approval via Inline Buttons, TG-4: Run Command Approval, TG-5: Plan Widget Interaction, TG-6: Send Messages (+4 more)

### Community 36 - "3. Message Format Specification"
Cohesion: 0.17
Nodes (12): 3.10 Ephemeral activity indicator, 3.11 Questionnaire (from state.questionnaire), 3.1 Human Message, 3.2 Assistant Message, 3.3 Tool Call, 3.4 Thought Block, 3.5 Plan Widget, 3.6 Run Command (+4 more)

### Community 37 - "server/index.ts"
Cohesion: 0.24
Nodes (6): logStream, checkLicense(), LICENSE_PATH, readStoredKey(), validateKey(), Transport
...
Cohesion: 0.22
Nodes (9): 11. Implementation Status, 12. Risks & Mitigations, 13. Future Roadmap, 14. Success Criteria, 5.1 Server → Client, 5.2 Client → Server, 5. Protocol — socket.io Events, 8. Configuration (+1 more)
...
Cohesion: 0.22
Nodes (9): 10. Success Criteria, 1.1 Problem Statement, 1.2 Goal, 1.3 Non-Goals, 1. Overview, 4. Command Reference, 6. Access Control, 9. Configuration (+1 more)
...
Cohesion: 0.33
Nodes (6): 8.1 Window/Tab Not Found, 8.2 Multiple Active Users, 8.3 Bot Restart, 8.4 Long Messages Split, 8.5 Callback Data Overflow, 8. Edge Cases
...
Cohesion: 0.40
Nodes (5): 5.1 Structure, 5.2 Mapping Storage, 5.3 Topic Lifecycle, 5.4 Active Topic Resolution, 5. Topic Mapping

### Community 71 - "7. Rate Limiting and Constraints"
Cohesion: 0.40
Nodes (5): 7.1 Telegram API Limits, 7.2 Rate Limit Implementation, 7.3 Initial Sync Throttling, 7.4 Message Batching, 7. Rate Limiting and Constraints
...
### Community 76 - "10. Key Technical Decisions"
Cohesion: 0.50
Nodes (4): 10.1 Custom CDP Client vs. Puppeteer, 10.2 CDP Input Domain for Text Entry, 10.3 Data-Attribute Extraction vs. Class-Based Selectors, 10. Key Technical Decisions
...
## Knowledge Gaps
- **558 isolated node(s):** `production`, `shared`, `JsonLogLine`, `TelegramAuth`, `PanelState` (+553 more)
...
- **Why does `grammy` connect `cursorRemote.telegram.impl` to `telegram-raw/index.ts`, `server/types.ts`?**
